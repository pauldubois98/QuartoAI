...
class PlaceNet(nn.Module):
    def __init__(self):
        ...
        self.boardIn1 = nn.Linear(4*4*4, 256)
        self.boardIn2 = nn.Linear(4*4*4, 256)
    ...
    def forward(self, board, pieces, piece):
        b1 = board[:, :, 0]*torch.moveaxis(board[:, :, 1:], 2, 0)
        b2 = board[:, :, 0]*(1-torch.moveaxis(board[:, :, 1:], 2, 0))

        # b1 = board[:,:,0]*np.moveaxis(board[:,:,1:], 2, 0)
        # b2 = board[:,:,0]*(1-(np.moveaxis(board[:,:,1:], 2, 0)))
        x0 = self.pieceIn(piece)
        x1 = self.boardIn1(torch.flatten(b1))
        x2 = self.boardIn2(torch.flatten(b2))
        y = self.piecesIn(pieces)
        z = F.relu(x0)+F.relu(x1)+F.relu(x2)+F.relu(y)
